...
class ITC_SVG_Upload_Svg {
    ...
    function wp_generate_attachment_metadata($metadata, $attachment_id) {
        if (get_post_mime_type($attachment_id) == 'image/svg+xml') {
            ...
            $svg_content = preg_replace('/<script\b[^>]*>(.*?)<\/script>/is', '', $svg_content); // Remove <script> tags
            $svg_content = preg_replace('/<style\b[^>]*>(.*?)<\/style>/is', '', $svg_content); // Remove <style> tags
            file_put_contents($svg_path, $svg_content); // Save sanitized SVG back
